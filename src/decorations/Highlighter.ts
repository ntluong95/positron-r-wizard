...
import { parseRBlocks } from "../parsers/rParser";
import { CodeBlock } from "../parsers/types";
import {
...
export class Highlighter {
  ...
  private registerConfigurationListener() {
    const disposable = vscode.workspace.onDidChangeConfiguration((e) => {
      const affectsScopeConfig = this.configSections.some((section) =>
        e.affectsConfiguration(section),
      );
      if (affectsScopeConfig) {
  ...
  private getBlockTree(document: vscode.TextDocument): BlockTree {
    if (!this.blockTree) {
      const blocks = parseRBlocks(document);
